...
- **Task: TASK-406 — World Render Layer (feature/world-system)**
  ...
  - **TASK-506 MASTERY PASS (qa/world-system @ 2ef4758, 2026-09-19)**: devastation decay was dead (rotating cursor = one visit per ~9min → blasts never faded; now a live-cell Set, full fade ≈2.5min) + occupation now clears the scorch VISUALLY (bucket+dirty+set); morale system was dead wiring (attackLogic consumed ctx.getMorale/onLosses since 406 but no provider) — now wired end-to-end with a 🚩±N% badge in the troops stat, ±15% swing, live-verified incl. all-6-bots at the 0.85 floor in total war; geoLand late-load safe re-apply (lead's flag — pre-spawn rebuild, in-game refusal documented); **geo-referenced coast repair: Dover + Gibraltar genuinely crossable on the real map now** (terrain binary over-waters Kent ~25km and Spain's south coast ~78km; two defect classes + coastal-band gate, lakes/rivers/deltas preserved — live-calibrated); render allocation churn −89% (frontier/heat persistent drawRange buffers, measured A/B vs cac6969); worldQaTest 13/13; cross-system regression ALL GREEN on :3006 (econ 17, ecm 5, sam 4, drone, missile 12, tank 4, navalDeep 13, visual 9, audio, dogfight/airstrike pass). Tests: world_render 56/56, water_mask 47/47. Remaining for @lead: bosporus/messina/bering straits = source-binary resolution limit (reproject pipeline).
